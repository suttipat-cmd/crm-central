...
### v1.3.0 — Legacy Lead Forms, Lead Channels and Business Types

- เพิ่ม SQL migration สำหรับ field จากฟอร์มจริงของ MKT/Sale: `legacy_account_id`, `lead_channel_id`, `business_type_id`, `address`, `current_gps_provider`, `phone_2`, `phone_3`, `email_2`
- เพิ่ม master data ใหม่ `lead_channels` และ `business_types` ให้ Admin จัดการได้
- ปรับฟอร์มสร้าง MKT Lead ให้ใกล้เคียงฟอร์มจริง: ลำดับ MKT read-only, ที่มา Lead = Marketing, ช่องทาง, แคมเปญ, Account ID เดิม, ผู้ติดต่อ, เบอร์, สินค้า และข้อมูล Lead
- ปรับฟอร์ม Sale สร้าง Lead เองให้รองรับ field จริง: บริษัท, ผู้ติดต่อ, ตำแหน่ง, เบอร์หลัก/2/3, อีเมล/อีเมล 2, Tax ID, ที่อยู่, ธุรกิจ, รายละเอียด, จำนวนรถ, สินค้า, GPS ปัจจุบัน, หมายเหตุ, อีเมลเซลล์ และช่องทาง
- `Account ID` จากระบบเดิมถูกเก็บเป็น `legacy_account_id` เท่านั้น ไม่ใช้เป็น primary key และไม่มีผลต่อ Supabase `accounts.id`
- `ช่องทาง` ใช้ master เดียวกันทั้ง MKT และ Sale ผ่าน `lead_channels`
- Sale assignment ของ MKT Lead ยังเป็น round-robin อัตโนมัติ และแสดง Sale owner หลังสร้างสำเร็จ
- ปรับ Account Overview/List/Search/Filter ให้รองรับช่องทาง, ธุรกิจ, Account ID เดิม, GPS ปัจจุบัน และข้อมูลติดต่อเพิ่มเติม
- ต้องรัน SQL migration v1.3.0 ก่อน deploy frontend v1.3.0
...
- เพิ่ม `#/my-work` สำหรับรวมงานที่ต้องทำต่อของผู้ใช้แต่ละ role
- เพิ่ม search, filter, sort และ pagination ให้ collection หลัก เช่น Leads, Accounts, Demo, Customers, Tasks และ Training
- ปรับ Account Detail เป็น tab แยก `ภาพรวม / Demo / Training / Customer / Tasks / History` เพื่อลดการ scroll ในหน้าเดียว
- เพิ่มหน้า Unauthorized และ Not Found แทนการ redirect เงียบ
- ปรับ Login/Pending copy ให้เป็นภาษาผู้ใช้ ลดคำ technical และเพิ่มคำแนะนำเมื่อเข้าไม่ได้
- เพิ่ม inline/form validation feedback สำหรับ form สำคัญ
- เปลี่ยน multi-select แบบ native เป็น checkbox/chip selector เพื่อใช้ง่ายขึ้นบน desktop/mobile
- เพิ่ม confirmation สำหรับ action เสี่ยง เช่น Mark Lost และ Mark Task Done
- เพิ่ม empty state ที่บอก next context ชัดขึ้น
- เพิ่ม responsive polish สำหรับ filter, navigation, tabs, table และ action buttons
- เพิ่ม accessibility improvement: label/input linkage ใน helper fields, aria-invalid, form error role alert และ topbar role chip

> v1.1.0 ไม่มี migration SQL ใหม่ โครงสร้างฐานข้อมูลยังใช้ schema v1.0.2 ได้
...
```text
MKT สร้าง Lead
→ กรอกข้อมูลจากฟอร์ม Marketing เช่น ช่องทาง, แคมเปญ, Account ID เดิม, ผู้ติดต่อ, เบอร์, Module และข้อมูล Lead
→ ระบบออก running_no เป็นตัวเลข เริ่มจาก 1
→ ระบบ assign Sale แบบ round-robin จาก profiles.role = sale และ is_active = true
→ ระบบแสดง Running No. และ Sale ที่ถูก assign หลัง Save
...
```text
Sale สร้าง Lead เอง
→ กรอกข้อมูลจากฟอร์ม Sale เช่น บริษัท, ผู้ติดต่อ, ตำแหน่ง, เบอร์หลัก/2/3, อีเมล/อีเมล 2, Tax ID, ที่อยู่, ธุรกิจ, รายละเอียด, จำนวนรถ, Module, GPS ปัจจุบัน และช่องทาง
→ ไม่มี Running No.
→ sale_owner_id = Sale ที่ login และสร้างรายการ
...
|---|---|
| accounts | sale_owner_id, mkt_created_by, lifecycle_stage, lost_from_stage, legacy_account_id, lead_channel_id, business_type_id |
...
- current_gps_provider text
- lead_channel_id uuid references lead_channels(id)

account_contacts added columns
- phone_2 text
- phone_3 text
- email_2 text
```

### 8.1 SQL Migration v1.3.0

Run this migration once after the base SQL setup and before deploying frontend v1.3.0.

```sql
-- Internal CRM Ops v1.3.0 Migration
-- Add legacy external fields, lead channels and business type master data.
-- Run after base schema v1.0.x/v1.2.x has already been installed.
...
alter table public.accounts
  ...
  add column if not exists current_gps_provider text,
  add column if not exists lead_channel_id uuid references public.lead_channels(id);
...
create index if not exists idx_accounts_legacy_account_id on public.accounts(legacy_account_id);
create index if not exists idx_accounts_business_type on public.accounts(business_type_id);
create index if not exists idx_accounts_lead_channel on public.accounts(lead_channel_id);

alter table public.business_types enable row level security;
alter table public.lead_channels enable row level security;

grant usage on schema public to authenticated;
grant all on public.business_types to authenticated;
grant all on public.lead_channels to authenticated;

drop policy if exists master_select_business_types on public.business_types;
create policy master_select_business_types on public.business_types
for select to authenticated
using (is_active = true or public.is_admin());

drop policy if exists master_write_business_types on public.business_types;
create policy master_write_business_types on public.business_types
for all to authenticated
using (public.is_admin())
with check (public.is_admin());

drop policy if exists master_select_lead_channels on public.lead_channels;
create policy master_select_lead_channels on public.lead_channels
for select to authenticated
using (is_active = true or public.is_admin());

drop policy if exists master_write_lead_channels on public.lead_channels;
create policy master_write_lead_channels on public.lead_channels
for all to authenticated
using (public.is_admin())
with check (public.is_admin());

drop function if exists public.create_mkt_lead(text, text, text, text, uuid, uuid, text, integer, uuid[]);
drop function if exists public.create_mkt_lead(text, text, text, text, text, uuid, uuid, uuid, text, integer, uuid[]);
drop function if exists public.create_sales_lead(text, text, text, text, uuid, text, integer, uuid[]);
drop function if exists public.create_sales_lead(text, text, text, text, text, text, text, text, text, text, text, uuid, uuid, text, integer, text, uuid[]);

create or replace function public.create_mkt_lead(
  p_legacy_account_id text default null,
  p_company_name text default null,
  p_contact_name text default null,
  p_phone text default null,
  p_email text default null,
  p_lead_source_id uuid default null,
  p_lead_channel_id uuid default null,
  p_campaign_id uuid default null,
  p_initial_note text default null,
  p_cars_estimate integer default null,
  p_module_ids uuid[] default '{}'::uuid[]
...
begin
  ...
  if p_lead_channel_id is null then
    raise exception 'lead channel is required';
  end if;

  if coalesce(
    nullif(trim(p_legacy_account_id), ''),
    nullif(trim(p_company_name), ''),
...
create or replace function public.create_sales_lead(
  ...
  p_initial_note text default null,
  p_cars_estimate integer default null,
  p_current_gps_provider text default null,
  p_module_ids uuid[] default '{}'::uuid[]
)
returns uuid
language plpgsql
...
begin
  ...
  if coalesce(
    ...
    nullif(trim(p_contact_name), ''),
    nullif(trim(p_phone), ''),
    nullif(trim(p_phone_2), ''),
    nullif(trim(p_phone_3), ''),
    nullif(trim(p_email), ''),
    nullif(trim(p_email_2), ''),
    nullif(trim(p_initial_note), '')
  ) is null then
    raise exception 'lead minimum data is required';
  end if;
  ...
  values (
    ...
    nullif(trim(p_initial_note), ''),
    p_cars_estimate,
    nullif(trim(p_current_gps_provider), ''),
    v_uid,
    v_uid
  )
  returning id into v_account_id;
  ...
  if p_initial_note is not null and trim(p_initial_note) <> '' then
    insert into public.account_activities (account_id, activity_type, title, content, created_by)
    values (v_account_id, 'sale_update', 'Sale initial detail', p_initial_note, v_uid);
  end if;
...
grant execute on function public.create_mkt_lead(text, text, text, text, text, uuid, uuid, uuid, text, integer, uuid[]) to authenticated;
grant execute on function public.create_sales_lead(text, text, text, text, text, text, text, text, text, text, text, uuid, uuid, text, integer, text, uuid[]) to authenticated;
...
3. Run base SQL setup above
4. Run SQL Migration v1.3.0 before deploying frontend v1.3.0
5. Run first admin seed block with real Auth UID
6. Go to app, login as first admin
7. In Admin page, set active Sale/MKT/CS/Manager users
8. Add master data: Lead Channels, Business Types, Lead Sources, Campaigns, Modules, Contact Statuses, Lost Reasons
...
git commit -m "release: internal crm ops v1.3.0"
git branch -M main
...
ไม่ต้องรัน SQL ใหม่ ถ้าฐานข้อมูลเคยรัน setup v1.0.2 / v1.0.1 แล้ว เพราะ v1.1.0 เป็น UX/frontend release เท่านั้น

### Files to deploy

ต้อง deploy ทับเฉพาะ 4 ไฟล์นี้:
...
### Manual checks หลัง deploy

```text
1. เปิดเว็บแล้ว hard refresh
2. Login ด้วย admin
3. ตรวจ Dashboard และ My Work
4. เข้า Leads / Accounts / Tasks / Training แล้วลอง search, filter, sort, pagination
5. เปิด Account Detail แล้วสลับ tab ภาพรวม / Demo / Training / Customer / Tasks / History
6. ทดสอบ form validation โดย submit Lead ว่าง
7. ทดสอบ Mark Lost / Mark Task Done ว่ามี confirmation
8. ทดสอบ role Sale เห็นเฉพาะ account ของตัวเอง
9. ทดสอบ CS เห็น Demo/Customer/Task ฝั่ง CS
10. ทดสอบมือถือหรือ responsive width ต่ำกว่า 760px
```

### Rollback

Frontend rollback ได้ด้วยการ revert commit v1.1.0:
...
Database rollback ไม่เกี่ยวข้องกับ release นี้ เพราะไม่มี migration SQL ใหม่
